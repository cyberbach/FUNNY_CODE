...
- Blueprint-интеграция через `ULocalSTTAudioCaptureComponent`
- C++ API с классами `ULocalSTTModel`, `ULocalSTTRecognizer`
- Настройки через Project Settings UI

## Быстрый старт

1. Скопируйте плагин в `Plugins/LocalSTTPlugin57/`
2. Поместите VOSK SDK в `Plugins/LocalSTTPlugin57/ThirdParty/vosk/`
3. Положите модель в `Plugins/LocalSTTPlugin57/Binaries/Win64/Models/`
4. В Blueprint: добавьте `Local STT Audio Capture` на актёр → `Initialize` → `StartRecording` → слушайте `OnFinalResult`
...
|-----------|----------|-----------------|
| **LocalSTTPlugin57** (плагин) | MIT | Andrey (cb) Mikheev |
