...
ULocalSTTPluginSettings::ULocalSTTPluginSettings()
	: ModelsDirectory(TEXT(""))
	, DefaultModelName(TEXT("vosk-model-small-ru-0.22"))
	, bEnableLocalSTTLogging(false)
	, VoskLogLevel(0)
	, bAutoAddModelsToPackaging(true)
{
}

ULocalSTTPluginSettings* ULocalSTTPluginSettings::Get()
{
	return StaticClass()->GetDefaultObject<ULocalSTTPluginSettings>();
}

FString ULocalSTTPluginSettings::GetLocalSTTModelsPath() const
{
	...
	// Предпочитаем папку внутри плагина: Plugins/LocalSTTPlugin57/Binaries/Win64/Models
	FString BaseDir;
	...
	else
	{
		...
		FString ProjectPluginBin = FPaths::Combine(FPaths::ProjectDir(), TEXT("Plugins"), TEXT("LocalSTTPlugin57"), TEXT("Binaries"), TEXT("Win64"), TEXT("Models"));
...
FString ULocalSTTPluginSettings::GetPluginBinPath() const
{
	TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("LocalSTTPlugin57"));
	if (Plugin.IsValid())
	{
...
FString ULocalSTTPluginSettings::GetDefaultModelPath() const
{
	return FPaths::Combine(GetLocalSTTModelsPath(), DefaultModelName);
}

bool ULocalSTTPluginSettings::IsVoskSdkInstalled() const
{
	TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("LocalSTTPlugin57"));
	if (Plugin.IsValid())
	{
...
bool ULocalSTTPluginSettings::IsModelInstalled(const FString& ModelName) const
{
	FString ModelsDir = GetLocalSTTModelsPath();
